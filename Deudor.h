...
public:
	...
	debt(string nom, string com) : persona(nom, com) {
	...
	void crear() {
		...
		if (cn.getConectar()) {
			string vQuery = "INSERT INTO deudores(deudorNombre,deudorDeuda)VALUES('" + nombre + "','" + compra + "');";
	...
	void readD()
	{
		...
		if (cn.getConectar()) {
			string ask = "select * from deudores";
